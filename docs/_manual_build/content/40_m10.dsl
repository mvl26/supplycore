...
BODY	**Vai trò:** **Kiểm soát Chất lượng** (Pharmacy/QC Officer) tra cứu lô và theo dõi tiến độ thu hồi; **Trưởng phòng Vật tư** (SupplyCore Manager) tạo, nộp và xử lý Thông báo Thu hồi cũng như mở Báo cáo Điều tra; **Thủ kho** tra cứu lô, ghi nhận số đã thu hồi và đã hủy. Quản trị Hệ thống có toàn quyền.
...
OL	Khối **Tồn kho hiện tại** cho tổng tồn còn lại và chi tiết theo từng kho. Khối **Sổ cái tồn kho** liệt kê mọi bút toán nhập/xuất của lô (xuôi thời gian), tổng Nhập và tổng Xuất; dòng đã hủy bị gạch ngang. Khối **Cấp phát bệnh nhân** cho biết lô đã dùng cho bệnh nhân nào, khoa nào, BHYT trả và bệnh nhân trả bao nhiêu.
...
NOTE	Lô đã xuất hết vẫn tra cứu được — Sổ cái và phần cấp phát vẫn hiện đầy đủ, chỉ Tồn kho hiện tại bằng 0.
...
OL	**Lưu** bản nháp, sau đó chạy chức năng **Nạp items bị ảnh hưởng** (populate). Hệ thống dò Sổ kho và bảng cấp phát để tự điền bảng **Items bị ảnh hưởng**: các dòng còn tồn trong kho (Loại vị trí = Warehouse) và các dòng đã cấp cho bệnh nhân (Loại vị trí = Patient), kèm số lượng đã cấp/tồn.
...
OL	Kiểm tra lại danh sách, sau đó **Nộp** (Submit) phiếu. Khi nộp, hệ thống khóa lô (đặt cờ chặn trên SC Batch kèm lý do "Recall …"): từ đây mọi phiếu cấp phát và xuất kho dùng lô này đều bị chặn, ghi nhận **Người duyệt** và **Thời điểm duyệt**, trạng thái chuyển sang **Issued**.
OL	Nếu có dòng bị ảnh hưởng là bệnh nhân, chạy **Thông báo nhân viên lâm sàng** (notify) để gửi cảnh báo tới Pharmacy Officer và Trưởng phòng Vật tư; các dòng bệnh nhân được đánh dấu **Đã báo BS** và phiếu ghi **Thời điểm báo BS điều trị**.
OL	Để gửi phiếu thu hồi cho từng khoa, dùng chức năng gửi theo khoa (notify_departments) — hệ thống gom các dòng theo **Khoa** để in/gửi thư.
NOTE	Nếu không xác định được khoa nào đã nhận, dùng chức năng rà soát cấp phát trong giai đoạn (audit theo khoảng ngày, mặc định 90 ngày gần nhất) để liệt kê toàn bộ ca cấp phát của vật tư rồi bổ sung thủ công.
...
UL	Thu hồi tạo bản ghi **SC-RCL-YYYY-#####**; khi nộp sẽ khóa lô (cờ chặn trên SC Batch) để dừng cấp phát/xuất kho.
...
UL	Truy xuất lô không tạo bản ghi mới — chỉ đọc Sổ kho, Phiếu nhập/Đơn mua/Phiếu KCS và Cấp phát để dựng lại lịch sử lô (xuôi và ngược).
...
UL	**Mẹo —** trước khi nộp Thu hồi, chạy lại Nạp items để chắc chắn đã gồm mọi nơi lô từng đến (kho và bệnh nhân).
H3	3.10.7	Liên quan
UL	Xem 3.3 — Tiếp nhận & Kiểm tra chất lượng (Phiếu nhập, Phiếu KCS — nguồn gốc của lô).
UL	Xem 3.7 — Cấp phát & BHYT (Cấp phát bệnh nhân — nơi lô đến tay người bệnh).
UL	Xem mục Tồn kho & FEFO (Sổ kho, khóa lô ảnh hưởng tới xuất kho/cấp phát).
