...
export const ACTIONS = {
  ...
  'SC Purchase Receipt': [
    ...
    { apiMethod: 'supplycore.m8_accounting.doctype.sc_purchase_invoice.sc_purchase_invoice.make_invoice_from_pr',
      apiNameArg: 'pr_name',
      label: 'Tạo Hoá đơn mua (PI)', icon: '🧾', variant: 'primary',
  ...
  'SC Payment Entry': [
    ...
    { apiMethod: 'supplycore.m8_accounting.doctype.sc_payment_entry.sc_payment_entry.auto_load_outstanding_invoices',
      ...
      label: 'Auto-load PI chưa thanh toán', icon: '📋', variant: 'primary',
  ...
  'SC Inventory Count Sheet': [
    { method: 'auto_load_items', label: 'Tự nạp items theo phạm vi', icon: '📋', variant: 'primary',
      when: (d) => d.docstatus === 0 && (d.status === 'Draft' || !d.status) },
    { method: 'start_counting', label: 'Bắt đầu đếm', icon: '▶', variant: 'primary',
      when: (d) => d.docstatus === 0 && d.status === 'Draft' },
    { method: 'get_count_sheet_print_data', label: 'Xem dữ liệu in phiếu',
      icon: '🖨️', variant: 'secondary',
  ...
  'SC Stock Reconciliation': [
    { method: 'load_from_count_sheet', label: 'Tải từ phiếu kiểm', icon: '⬇️', variant: 'secondary',
  ...
  'SC Recall Notice': [
    ...
    { method: 'update_recovery', label: 'Cập nhật thu hồi (dòng)', icon: '📝', variant: 'secondary',
  ...
  'SC Alert': [
    ...
    { method: 'action_create_purchase_order',
      label: '🛒 Tạo PO khẩn', icon: '🛒', variant: 'primary',
    ...
    { method: 'action_create_material_request',
      label: '📝 Tạo Yêu cầu mua (MR)', icon: '📝', variant: 'secondary',
      when: (d) => !d.resolved && d.alert_type === 'low_stock',
    ...
    { method: 'action_priority_dispense',
      label: '⚡ Ưu tiên cấp phát', icon: '⚡', variant: 'warning',
    ...
    { method: 'action_contact_supplier',
      label: '📧 Liên hệ NCC', icon: '📧', variant: 'primary',
    ...
    { method: 'action_quarantine_batch',
      label: '🚫 Cách ly lô', icon: '🚫', variant: 'danger',
      when: (d) => !d.resolved &&
    ...
    { method: 'action_create_payment',
      label: '💳 Tạo Phiếu thanh toán', icon: '💳', variant: 'primary',
